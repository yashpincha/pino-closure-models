...
class FCGram(nn.Module):
    ...
    def load_matrices(self):
        ...
        if not filepath.exists():
            ...
            filepath = Path('/global/homes/y/ypincha/pino-closure-models/ns1w/fcgram_matrices/FCGram_data_d5_c25.npz')
...
class FC3D(object):
    ...
    def diff_t(self, u, domain_length_t = 1):

        nt = u.size(1)
        nx = u.size(2)
        ny = u.size(3)

        hy = domain_length_t / (nt - 1)
        fourPtsy = nt + self.C
        prdy = fourPtsy * hy
        u = u.double()
        ...
        if fourPtsy % 2 == 0:
                ...
                k_y = torch.cat((torch.arange(start = 0, end = k_max + 1, step = 1, device = self.device),
                    torch.arange(start = - k_max + 1, end = 0, step = 1, device = self.device)), 0).reshape(fourPtsy, 1,1).repeat(1, nx,ny)
        else:
                k_max = int((fourPtsy - 1) / 2)
                k_y = torch.cat((torch.arange(start = 0, end = k_max + 1, step = 1, device = self.device),
                    torch.arange(start = - k_max, end = 0, step = 1, device = self.device)), 0).reshape(fourPtsy, 1,1).repeat(1, nx,ny)

        der_coeffsy = 1j * 2.0 * np.pi / prdy * k_y

        # compute derivatives along the y-direction
        # First produce the periodic extension
        y1 = torch.einsum("iklt,jk->ijlt", torch.einsum("iklt,kj->ijlt", u[:, -self.d:, :], self.Q), self.A)
        y2 = torch.flip(torch.einsum("iklt,jk->ijlt",torch.einsum("iklt,kj->ijlt",torch.flip(u[:, :self.d, :], dims=(1,)), self.Q), self.A), dims=(1,))
        # Compute the derivative of the extension
